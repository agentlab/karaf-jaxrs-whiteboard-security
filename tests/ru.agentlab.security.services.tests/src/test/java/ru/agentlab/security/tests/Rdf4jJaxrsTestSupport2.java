...
public class Rdf4jJaxrsTestSupport2 {
     ...
     * @param principals The principals (e.g. RolePrincipal objects) to run the
     *                   command under
     * @return
     */
    ...
    public <T> void assertServiceAvailable(Class<T> type) {
        Assert.assertNotNull(getOsgiService(type));
    }

    public <T> void assertServiceAvailable(Class<T> type, long timeout) {
        Assert.assertNotNull(getOsgiService(type, timeout));
    }

    public <T> void assertServiceAvailable(Class<T> type, String filter, long timeout) {
    ...
    private void waitForCommandService(String command) {
        // the commands are represented by services. Due to the asynchronous nature of
        // services they may not be
        // immediately available. This code waits the services to be available, in their
        // secured form. It
        // means that the code waits for the command service to appear with the roles
        // defined.
    ...
    /**
     * Provides an iterable collection of references, even if the original array is
     * null
     */
    @SuppressWarnings("rawtypes")
     ...
     * a single feature. So we need to make sure we uninstall all features that were
     * newly installed.
     *
